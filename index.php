...
EOS;
  ...
  if(!$res){
    ...
    while ($row = mysql_fetch_assoc($res)) {
      ...
      echo('<td><A Href="confirm.php?problem_id='.$row['problem_id'].'">削除</A></td>');
      echo "</tr>";
    }
  }
  echo "</table>";
...
if(isset($_GET["see_prob"])){
  ...
  if(!$res){
    ...
    echo "</tr>";
    // 最大選択回数と選択コストレートを取得
    $sql = sprintf("SELECT selection_rate, max_selection FROM problem_info WHERE problem_id=%d", intval($_GET["see_prob"]));
    $select_res = mysql_query($sql);
    $row = mysql_fetch_assoc($select_res);
    $selection_rate = $row["selection_rate"];
    $max_selection = $row["max_selection"];
    while ($row = mysql_fetch_assoc($res)) {
      // 最大選択回数を越えた解答を赤くする
      if ($row["selection_cost"] > $selection_rate * $max_selection) {
        echo '<tr class="red">';
      } else {
        echo "<tr>";
      }
      ...
      echo('<td>'.$row['time'].'</td>');
      echo('<td><A Href="confirm.php?score_id='.$row['score_id'].'">削除</A></td>');
      echo "</tr>";
    }
    echo "</table>";
  }
